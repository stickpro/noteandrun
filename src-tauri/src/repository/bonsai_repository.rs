...
impl<'a, TData> BonsaiRepository<'a, TDate>
    where Tdate: SerializedCollection<Contents=TData> +
...
impl <'a, TData> Repository<TData> for BonsaiRepository<'a, TData>
    ...
    async fn insert(&self, data: TData, id: &str) -> TDate {
        let new_document = data.push_into_async(self.db).await.unwrap();
        new_document.contents
    }
    async fn edit(&self, data: TData, id: &str) -> TData {
